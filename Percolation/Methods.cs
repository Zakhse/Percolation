...
namespace Percolation
{
	class Methods
	{
		...
		public static PercolationCell[,] CreatePercolationMatrix(uint size, double probability)
		{
			PercolationCell[,] matrix = new PercolationCell[size, size];
		...
		public static bool CheckPercolation(PercolationCell[,] matrix, bool turnAllGroupsToRight=false)
		{
			...
			//int[] group = new int[0];
			int[] group = new int[matrix.GetLength(0)*matrix.GetLength(1)/2];
			int nextGroup = 0;
